...
DEFAULT_OUTPUT_PATH = r"D:\Project\ocr\output"
CONFIG_FILE = "config.json"
# Poppler path for pdf2image (None = use system poppler)
# สามารถปรับค่าจาก Settings ได้ เพื่อให้ใช้งานบนหลายเครื่องได้ง่าย
POPPLER_PATH = None  # Example: r"C:\poppler\Library\bin"
# Tesseract path for pytesseract (None = use system PATH)
TESSERACT_PATH = r"C:\Program Files\Tesseract-OCR\tesseract.exe"  # Default Windows install path
...
def enrich_vendor_data(df):
    """
    Auto-populate Vendor Name and Vendor code based on VendorID_OCR and Branch_OCR
    """
    try:
        # Find relevant columns
        cols = df.columns
        col_vid = find_column_name(cols, ["vendor", "id"]) or "VendorID_OCR"
        col_branch = find_column_name(cols, ["branch"]) or "Branch_OCR"
        col_vcode = find_column_name(cols, ["vendor", "code"]) or "Vendor code"
        col_vname = find_column_name(cols, ["vendor", "name"]) or "Vendor Name"
        
        # Check if source columns exist
        if col_vid not in cols or col_branch not in cols:
            return df
            
        # Check if target columns need update (if they don't exist, create them later if needed, 
        # but usually they are created before calling this)
        
        # Load master data
        master = load_vendor_master()
        if master is None:
            return df
            
        # Prepare master lookup dicts for faster access
        # Key: (vendor_id_cleaned, branch_cleaned) -> {code, name}
        lookup_map = {}
        
        # Find master columns
        m_vid_col = 'เลขประจำตัวผู้เสียภาษี'
        m_br_col = 'สาขา'
        m_code_col = find_column_name(master.columns, ["vendor", "code"])
        m_name_col = 'ชื่อบริษัท'
        
        if m_vid_col not in master.columns or m_br_col not in master.columns:
            return df
            
        for _, row in master.iterrows():
            vid = str(row[m_vid_col]).strip()
            br = str(row[m_br_col]).strip()
            code = str(row[m_code_col]).strip() if m_code_col and pd.notna(row[m_code_col]) else ""
            name = str(row[m_name_col]).strip() if m_name_col in master.columns and pd.notna(row[m_name_col]) else ""
            
            lookup_map[(vid, br)] = {'code': code, 'name': name}
            
        # Iterate and update
        rows_updated = 0
        for idx in df.index:
            # Get current values
            curr_vid = str(df.at[idx, col_vid]).strip() if pd.notna(df.at[idx, col_vid]) else ""
            curr_br = str(df.at[idx, col_branch]).strip() if pd.notna(df.at[idx, col_branch]) else ""
            
            # Clean for lookup
            clean_vid = re.sub(r'\D', '', curr_vid)
            
            clean_br = curr_br
            if re.search(r'(สำนักงานใหญ่|สนญ|Head\s*Office|H\.?O\.?)', clean_br, re.IGNORECASE):
                clean_br = "00000"
            elif clean_br.isdigit():
                clean_br = clean_br.zfill(5)
                
            # Lookup
            key = (clean_vid, clean_br)
            if key in lookup_map:
                info = lookup_map[key]
                
                # Update Code if empty
                curr_code = str(df.at[idx, col_vcode]).strip() if col_vcode in df.columns and pd.notna(df.at[idx, col_vcode]) else ""
                if (not curr_code or curr_code.lower() == 'nan') and info['code']:
                    if col_vcode not in df.columns: df[col_vcode] = ""
                    df.at[idx, col_vcode] = info['code']
                    rows_updated += 1
                    
                # Update Name if empty
                curr_name = str(df.at[idx, col_vname]).strip() if col_vname in df.columns and pd.notna(df.at[idx, col_vname]) else ""
                if (not curr_name or curr_name.lower() == 'nan') and info['name']:
                    if col_vname not in df.columns: df[col_vname] = ""
                    df.at[idx, col_vname] = info['name']
                    rows_updated += 1
        
        if rows_updated > 0:
            # st.toast(f"Updated vendor info for {rows_updated} fields", icon="✅")
            pass
            
        return df
    except Exception as e:
        st.error(f"Error enriching vendor data: {e}")
        return df
    ...
    with col_list:
        if st.session_state.df_data is None:
            ...
            if uploaded_file:
                ...
                # Enrich with Vendor Data immediately on load
                st.session_state.df_data = enrich_vendor_data(st.session_state.df_data)
            ...
            with c_sheet:
                ...
                if new_sheet != curr:
                    ...
                    # Enrich with Vendor Data immediately on sheet change
                    df = enrich_vendor_data(df)
            ...
            with c_gen:
                if st.button("⚙️ Gen SAP", help="สร้างไฟล์ขึ้น SAP", use_container_width=True):
                    try:
                        # รัน TempleteSAP.py เพื่อสร้างไฟล์ SAP
                        script_path = os.path.join(SCRIPT_DIR, "TempleteSAP.py")
                        result = subprocess.run(
                            ["python", script_path],
                            capture_output=True,
                            text=True,
                        ...
                        if result.returncode == 0:
                            st.toast("สร้างไฟล์ SAP เรียบร้อย! ตรวจสอบที่ D:\\Project\\ocr\\output\\TestSAP.txt", icon="✅")
                        else:
                            st.error(f"สร้างไฟล์ SAP ไม่สำเร็จ: {result.stderr}")
                    except Exception as e:
